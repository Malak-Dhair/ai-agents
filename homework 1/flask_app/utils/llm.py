...
MASTER_TEMPLATE = """You are an AI assistant representing a software developer on their resume website.
You speak directly to recruiters, hiring managers, and engineers who are evaluating this developer for a job.
...
- Use only the provided context to answer factually about the candidate.
- Do not make up information that is not in the context.
"""
from flask import current_app

def fill_template(system_instruction, context, chat_history, user_message):
    """Fills the MASTER_TEMPLATE with the provided context and history."""
...
def handle_ai_chat_request(user_message, chat_history=""):
    """
    Routes the request to the appropriate expert and generates the LLM response.
    """
    db = current_app.db
    
    # 1. جلب سياق السيرة الذاتية من قاعدة البيانات
    context = str(db.getResumeData())
    
    # 2. تحديد الخبير (يمكن تعيينه افتراضياً كـ orchestrator أو اختيار خبير محدد)
    role_data = db.get_role("orchestrator")
    system_instruction = role_data["system_instruction"]
    
    # 3. دمج القالب مع البيانات
    ...
    # 4. إرسال الطلب لنموذج الذكاء الاصطناعي
    response = send_message(full_prompt)
    return response
