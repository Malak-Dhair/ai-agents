...
class database:
    ...
    def get_role(self, role_name):
        """
        Fetch system instructions for a given role from llm_roles.
        Returns the role data or a default fallback instruction.
        """
        rows = self.query("SELECT role_name, system_instruction FROM llm_roles WHERE role_name = ?", (role_name,))
    ...
    def create_llm_roles_table(self):
        self.query("""
    ...
    def seed_llm_roles(self):
        roles = [
            ('architect', 'You analyze technical architecture, routes, database schemas, and system design.'),
            ('writer', 'You write polished, professional responses representing the software developer based on resume context.'),
            ('evaluator', 'You evaluate proposed answers for accuracy against context and check for any hallucinations.'),
            ('orchestrator', 'You route the user request to the correct specialist role or combine their insights to answer accurately.')
        ]
        for role, instruction in roles:
            self.query("""
                INSERT OR REPLACE INTO llm_roles (role_name, system_instruction)
                VALUES (?, ?)
            """, (role, instruction))
